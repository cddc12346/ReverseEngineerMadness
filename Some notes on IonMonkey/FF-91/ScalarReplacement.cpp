...
static bool IsObjectEscaped(MInstruction* ins, const Shape* shapeDefault) 
{
  ...
  JitSpew(JitSpew_Escape, "In IsObjectEscaped I added (line 162)\n");

  //ins is akin to the node
  JitSpewDef(JitSpew_Escape, "Check object\n", ins);

  JitSpewIndent spewIndent(JitSpew_Escape);

  //first time its called, we give it a shape
  const Shape* shape = shapeDefault;
  if (!shape) 
  {
    if (ins->isNewPlainObject()) 
    {
      JitSpew(JitSpew_Escape, "In isNewPlainObject, first time its called, giving it a shape\n");
  ...
  // ins->usesBegin and ins->usesEnd probably means the number of times the object is used
  for (MUseIterator i(ins->usesBegin()); i != ins->usesEnd(); i++) 
  {
    ...
    MDefinition* def = consumer->toDefinition();
    JitSpewDef(JitSpew_Escape, "Current def = \n", def);
    switch (def->op()) 
    {
      case MDefinition::Opcode::StoreFixedSlot:
      case MDefinition::Opcode::LoadFixedSlot:
        // Not escaped if it is the first argument.
        JitSpew(JitSpew_Escape, "In Opcode::Store/LoadFixedSlot\n");
        if (def->indexOf(*i) == 0) 
        {
          JitSpew(JitSpew_Escape, "In Opcode::Store/LoadFixedSlot first argument\n");
          break;
        }
        JitSpew(JitSpew_Escape, "Its not the first argument\n");
        JitSpewDef(JitSpew_Escape, "is escaped by\n", def);
        return true;

      //PostWriteBarrier does nothing
      case MDefinition::Opcode::PostWriteBarrier:
      {
        JitSpew(JitSpew_Escape, "In Opcode::PostWriteBarrier\n");
        break;
      }
      ...
      case MDefinition::Opcode::GuardShape: 
      {
        ...
        JitSpew(JitSpew_Escape, "In Opcode::GuardShape\n");
        ...
        //in opcode guardshape, it will check if the shape
        if (IsObjectEscaped(def->toInstruction(), shape)) 
        {
          JitSpewDef(JitSpew_Escape, "is indirectly escaped by\n", def);
          return true;
      ...
      default:
      {
        JitSpew(JitSpew_Escape, "In default\n");
        JitSpewDef(JitSpew_Escape, "is escaped by\n", def);
        return true;
      }
    }
...
bool ScalarReplacement(MIRGenerator* mir, MIRGraph& graph) {
       ...
       block != graph.rpoEnd(); block++) 
  {
    ...
    //iterating through all the instructions
    for (MInstructionIterator ins = block->begin(); ins != block->end(); ins++)
    {
      //if object not escaped
