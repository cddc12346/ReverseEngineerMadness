...
bool CanOverflowSigned32(const Operator* op, Type left, Type right,
  ...
  // left = [MinusZero, (-1,-1)] , right = [(-0x80000000, 1)]
  // Without MinusZero, this Intersect with Signed32 will miss the MinusZero type
  // In the event where left is MinusZero, the below intersect will make left lose its Type, left will get a None type
  left = Type::Intersect(left, Type::Signed32(), type_zone);
  right = Type::Intersect(right, Type::Signed32(), type_zone);
  // left = [(-1,-1)] , right = [(-0x80000000, 1)]

  //if left or right isNone, then it is just itself, so it will definitely not overflow
  if (left.IsNone() || right.IsNone()) 
  {
    PrintF("Hit left.IsNone / right.IsNone\n");
    return false;
  }

  //In kSpeculativeSafeIntegerSubtract:
  //left.Max() = -1, left.Min() = -1, lost the MinusZero type
  //right.Max() = 1, right.Min() = -0x80000000
  //left.Max - right.Max = 0
  //left.Min - right.Min = -0x7FFFFFFF
  //returns (CanOverFlowSigned32 == False)
  //missing the MinusZero type!!!
  switch (op->opcode()) {
    case IrOpcode::kSpeculativeSafeIntegerAdd:
      PrintF("Hit kSpeculativeSafeIntegerAdd\n");
      //if left.Max + right.Max > kMaxInt then it is possible to overflow
      return (left.Max() + right.Max() > kMaxInt) ||
             (left.Min() + right.Min() < kMinInt);

    case IrOpcode::kSpeculativeSafeIntegerSubtract:
      PrintF("Hit kSpeculativeSafeIntegerSubtract\n");
...
#undef DECLARE_CASE
  ...
  void VisitSpeculativeIntegerAdditiveOp(Node* node, Truncation truncation,
                                         SimplifiedLowering* lowering) {
    PrintF("Hit VisitSpeculativeIntegerAdditiveOp\n");
    ...
    if (left_upper.Is(type_cache_->kAdditiveSafeIntegerOrMinusZero) &&
      ...
      if (truncation.IsUnused()) {
        PrintF("Truncation unused, return\n");
          ...
          truncation.IsUsedAsWord32()) 
      {
        ...
        PrintF("Print GetUpperBound Typed Signed32, return\n");
        return;
      }
    }
    PrintF("Didnt escape\n");
    ...
    //lower<T> is referring to the RunLowerPhase, so CanOverflowSigned32 is only hit in the LoweringPhase
    //if CanOverflow returns false?
    if (lower<T>()) {
      if (truncation.IsUsedAsWord32() ||
        ...
        //if CanOverflow returns True then change to Int32OverflowOp
        PrintF("It is possible to overflow, we use the ChangeToInt32Overflow\n");
